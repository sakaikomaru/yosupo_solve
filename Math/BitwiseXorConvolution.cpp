...
template<typename T>
void fmtXOR(vector<T>& f, int m) {
  fztXOR(f, m);
  T invM = T(1) / T(m);
  for (int i = 0; i < m; i++) f[i] *= invM;
}
template<typename T>
void XOR_convolution(vector<T>& a, vector<T>& b, int m) {
  fztXOR(a, m);
  fztXOR(b, m);
