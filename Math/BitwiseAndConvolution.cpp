...
template<typename T>
void fztAND(vector<T>& a, int m) { for (int d = 1; d < m; d <<= 1) REP(mask,m) if ((mask & d) == 0) a[mask] += a[mask | d]; }
template<typename T>
void fmtAND(vector<T>& a, int m) { for (int d = 1; d < m; d <<= 1) REP(mask,m) if ((mask & d) == 0) a[mask] -= a[mask | d]; }
template<typename T>
void AND_convolution(vector<T>& a, vector<T>& b, int m) {
  fztAND(a,m);
  fztAND(b,m);
  REP(i,m) a[i] *= b[i];
  fmtAND(a,m);
